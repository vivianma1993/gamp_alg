...
for iSNR=1:length(SNRs)
    ...
    for iCh=1:N_ch % one SNR value corresponding to 1e4 times sending
        ...
        %% MIMO channel matrix
        H=1/sqrt(2)/sqrt(Nt) *(randn(Nr,Nt)+1i*randn(Nr,Nt));%generate Nt*Nr H randn create mean=0,delta=1 matrix
...
%         ser_siso(iSNR) = symerrNr_siso/(N_ch);
        ...
        brrNr_ZF = brrNr_ZF+sum(sum(double(tx_bits~= rx_bits_zf_hard)));% sum of bit error
