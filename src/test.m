...
channelH = ones(2,2);%generate Nt*Nr H randn create mean=0,delta=1 matrix
n=1/sqrt(2) *sqrt(noise_var)*(randn(Nr,1)+1i*randn(Nr,1));
rxSig = channelH*s+n; % received signal vector    
...
if(ordering)
    ...
    b_est_hard_SIC(detected_stream_index,1)=b_est;
